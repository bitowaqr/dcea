  ...
  dashboardSidebar(
    ...
    sidebarMenu(
      ...
      menuItem(
        ...
        autonumericInput("incQALYs_c1", "Inc. QALYs per recipient", 0.02,  currencySymbol = "",decimalPlaces = 5),
        ...
        div(
          ...
          div(
            ...
            tags$table(
              ...
              tags$tr(
                tags$td(class="ps-4",tip(
                  str = "INHB",
                  tip = "Incremental Net Health Benefit (INHB) - the total gain in healthy years across the whole English population."
                )),
                tags$td(class = "text-center", textOutput("nhb",inline = T), "QALYs")
              ),
...
server = function(input, output, session){ 
  ...
  output$report_download <- downloadHandler(
    ...
    content = function(file) {
      ...
      if(input$intervention_type == "Risk factor population"){
        input_names = input_names[!(names(input_names) == "Intervention disease category (ICD-10)")]
      } else {
        input_names = input_names[!(names(input_names) == "Intervention risk factor")]
      }
      ...
      isolate({
        ...
        ratioHOC = formatC(input$ratioHOC,digits = 0, big.mark = ",",format = "f")
      ...
      report_params <- list(
        ...
        marginal_productivity = ratioHOC
        )
